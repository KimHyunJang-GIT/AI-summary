...
class LNPChat:
    ...
    translator: Optional[GoogleTranslator] = field(init=False, default=None)
    ...
    def ask(self, query: str, topk: Optional[int] = None) -> Dict[str, Any]:
        ...
        if self.translator:
            try:
                query_for_search = self.translator.translate(query)
        ...
        for i, h in enumerate(hits, 1):
            ...
            if h.get("preview"):
                answer_lines.append(f"   미리보기 (영문): {h['preview']}")
        if not hits:
